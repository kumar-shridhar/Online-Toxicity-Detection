...
class InstagramScraper(object):
    ...
    def safe_get(self, *args, **kwargs):
        ...
        while True:
            ...
            try:
                ...
                if content_length is not None and len(response.content) != int(
                    content_length
                ):
    ...
    def get_dst_dir(self, username):
        ...
        #         dst = "/home/MG10/Desktop/Scraping_script/All_output/" + username
    ...
    def get_last_scraped_timestamp(self, username):
        if self.latest_stamps_parser:
            try:
                return self.latest_stamps_parser.getint(
                    LATEST_STAMPS_USER_SECTION, username
                )
    ...
    def query_followings_gen(self, username, end_cursor=""):
        ...
        user = self.deep_get(
            self.get_shared_data(username), "entry_data.ProfilePage[0].graphql.user"
        )
        id = user["id"]
                ...
                for item in tqdm.tqdm(
                    ...
                    if self.filter_locations:
                        if (
                            ...
                            or self.get_key_from_value(
                                self.filter_locations, item["location"].get("id")
                            )
                            is None
                        ):
                            continue
                    ...
                    if self.include_location and "location" not in item:
                        media_exec.submit(
                            self.worker_wrapper, self.__get_location, item
                        )
    ...
    def query_location_gen(self, location):
        return self.__query_gen(
            QUERY_LOCATION, QUERY_LOCATION_VARS, "location", location
        )
        ...
        if node["is_video"] and "video_url" in node:
            ...
            if details:
                if "__typename" in details and details["__typename"] == "GraphVideo":
                    ...
                    "__typename" in details and details["__typename"] == "GraphSidecar"
                ):
                    urls = []
                ...
                try:
                    ...
                    if future_to_item:
                        for future in tqdm.tqdm(
                            ...
                            if future.exception() is not None:
                                self.logger.error(
                                    "Media at {0} generated an exception: {1}".format(
                                        item["urls"], future.exception()
                                    )
                                )
                            else:
                                timestamp = self.__get_timestamp(item)
    ...
    def get_profile_pic(self, dst, executor, future_to_item, user, username):
        ...
        if (
            ...
            or os.path.isfile(dst + "/" + item["urls"][0].split("/")[-1]) is False
        ):
            for item in tqdm.tqdm(
                [item],
    ...
    def get_stories(self, dst, executor, future_to_item, user, username):
        ...
        if self.logged_in and (
            ...
            for item in tqdm.tqdm(
                ...
                if self.story_has_selected_media_types(item) and self.is_new_media(
                    item
                ):
    ...
    def get_broadcasts(self, dst, executor, future_to_item, user):
        ...
        if self.logged_in and "broadcast" in self.media_types:
            ...
            for item in tqdm.tqdm(
                broadcasts,
                desc="Searching {0} for stories".format(user["username"]),
                unit=" media",
                disable=self.quiet,
            ):
    ...
    def get_media(self, dst, executor, future_to_item, user):
        ...
        if (
            "image" not in self.media_types
            and "video" not in self.media_types
            and "none" not in self.media_types
        ):
            return
        ...
        for item in tqdm.tqdm(
            ...
            if self.filter:
                if "tags" in item:
                    ...
                    if (
                        self.has_selected_media_types(item)
                        and self.is_new_media(item)
                        and filtered
                    ):
                        item["username"] = username
    ...
    def get_shared_data(self, username=""):
        ...
        if resp is not None and "_sharedData" in resp:
            try:
                shared_data = resp.split("window._sharedData = ")[1].split(
                    ";</script>"
                )[0]
    ...
    def __fetch_stories(self, url, fetching_highlights_metadata=False):
        ...
        if resp is not None:
            ...
            if (
                retval["data"]
                and "reels_media" in retval["data"]
                and len(retval["data"]["reels_media"]) > 0
                and len(retval["data"]["reels_media"][0]["items"]) > 0
            ):
                items = []

                for reel_media in retval["data"]["reels_media"]:
                    items.extend(
                        [self.set_story_url(item) for item in reel_media["items"]]
                    )
    ...
    def fetch_highlight_stories(self, user_id):
        ...
        if resp is not None:
            ...
            if (
                retval["data"]
                and "user" in retval["data"]
                and "edge_highlight_reels" in retval["data"]["user"]
                and "edges" in retval["data"]["user"]["edge_highlight_reels"]
            ):
                higlight_stories_ids = [
                    item["node"]["id"]
                    for item in retval["data"]["user"]["edge_highlight_reels"]["edges"]
                ]
                ...
                for ids_chunk in ids_chunks:
                    stories.extend(
                        self.__fetch_stories(
                            HIGHLIGHT_STORIES_REEL_ID_URL.format(
                                "%22%2C%22".join(str(x) for x in ids_chunk)
                            ),
    ...
    def fetch_broadcasts(self, user_id):
        ...
        if resp is not None:
            ...
            for broadcast in retval["post_live_item"]["broadcasts"]:
                ...
                best_video_quality = (
                    video_adaptation_set.get("maxWidth"),
                    video_adaptation_set.get("maxHeight"),
                )
                video_element = video_adaptation_set.find(
                    './/*[@width="{0}"][@height="{1}"]/{2}BaseURL'.format(
                        best_video_quality[0], best_video_quality[1], xmlns
                    )
                )
    ...
    def story_has_selected_media_types(self, item):
        ...
        if (
            item["__typename"] == "GraphStoryImage"
            and "story-image" in self.media_types
        ):
            return True
        if (
    ...
    def extract_tags(self, item):
        ...
        if "caption" in item and item["caption"]:
            ...
            "edge_media_to_caption" in item
            and item["edge_media_to_caption"]
            and item["edge_media_to_caption"]["edges"]
        ):
    ...
    def download(self, item, save_dir="./"):
        ...
        if self.filter_locations:
            save_dir = os.path.join(
                save_dir,
                self.get_key_from_value(self.filter_locations, item["location"]["id"]),
            )
                        ...
                        while True:
                            ...
                            try:
                                ...
                                if (
                                    ...
                                    raise PartialContentException(
                                        "Got first {0} bytes from {1}".format(
                                            downloaded, total_length
                                        )
                                    )
                            ...
                            # Please do not add os.remove here.
                                ...
                                if downloaded - downloaded_before > 0:
                                    ...
                                    self.logger.warning(
                                        "Continue after exception {0} on {1}".format(
                                            repr(e), media
                                        )
                                    )
                                ...
                                if retry < MAX_RETRIES:
                                    self.logger.warning(
                                        "Retry after exception {0} on {1}".format(
                                            repr(e), media
                                        )
                                    )
                                    self.sleep(retry_delay)
            ...
            try:
                ...
                template_values = {
                    ...
                    "datetime": time.strftime(
                        "%Y%m%d %Hh%Mm%Ss", time.localtime(self.__get_timestamp(item))
                    ),
                    "date": time.strftime(
                        "%Y%m%d", time.localtime(self.__get_timestamp(item))
                    ),
                    "year": time.strftime(
                        "%Y", time.localtime(self.__get_timestamp(item))
                    ),
                    "month": time.strftime(
                        "%m", time.localtime(self.__get_timestamp(item))
                    ),
                    "day": time.strftime(
                        "%d", time.localtime(self.__get_timestamp(item))
                    ),
                    "h": time.strftime(
                        "%Hh", time.localtime(self.__get_timestamp(item))
                    ),
                    "m": time.strftime(
                        "%Mm", time.localtime(self.__get_timestamp(item))
                    ),
                    "s": time.strftime(
                        "%Ss", time.localtime(self.__get_timestamp(item))
                    ),
                }
